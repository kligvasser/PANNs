...
def create_indexes(args):
    ...
    with h5py.File(waveforms_hdf5_path, "r") as hr:
        with h5py.File(indexes_hdf5_path, "w") as hw:
            ...
            hw.create_dataset(
                "index_in_hdf5", data=np.arange(audios_num), dtype=np.int32
            )
...
def combine_full_indexes(args):
    ...
    paths = [
        path
        for path in paths
        if ("train" in path and "full_train" not in path and "mini" not in path)
    ]
    ...
    with h5py.File(full_indexes_hdf5_path, "w") as full_hf:
        full_hf.create_dataset(
            name="audio_name", shape=(0,), maxshape=(None,), dtype="S20"
        )
        ...
        full_hf.create_dataset(
            name="hdf5_path", shape=(0,), maxshape=(None,), dtype="S200"
        )

        full_hf.create_dataset(
            name="index_in_hdf5", shape=(0,), maxshape=(None,), dtype=np.int32
        )
...
if __name__ == "__main__":
    ...
    parser_combine_full_indexes.add_argument(
        "--indexes_hdf5s_dir",
        type=str,
        required=True,
